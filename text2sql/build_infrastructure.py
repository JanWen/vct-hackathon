...
def create_crawler():
    ...
    classifier_name = glue_classifier_name # Name of the classifier
    json_path = '$*'  # JSON path that identifies elements of the array

    # Create the classifier
    response = glue.create_classifier(
        JsonClassifier={
            'Name': classifier_name,
            'JsonPath': json_path  # You can customize this to fit your JSON structure
        }
    )
...
# Once the needed IAM role is created, we can use the bedrock agent client to create a new agent. To do so we use the `create_agent` function. It requires an agent name, underline foundation model and instruction. You can also provide an agent description. Note that the agent created is not yet prepared. We will focus on preparing the agent and then using it to invoke actions and use other APIs
    ...
    the answers to questions about Pro Valorant Team and Players from the VCT Challengers league that you are asked. You first need to get the schemas for the table in the database to then query the 
